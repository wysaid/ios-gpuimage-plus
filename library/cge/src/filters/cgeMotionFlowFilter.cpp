...
//        glViewport(0, 0, m_width, m_height);
    ...
    CGEMotionFlow2Filter::CGEMotionFlow2Filter() : m_sizeScaling(1.0f), m_dSizeScaling(0.1f), m_dsMost(1.8f), m_continuouslyTrigger(false)
    {
        
    }
    
    void CGEMotionFlow2Filter::pushFrame(GLuint texture)
    {
        m_sizeScaling += m_dSizeScaling;
        
        if(m_sizeScaling >= m_dsMost)
        {
            m_dSizeScaling = -fabsf(m_dSizeScaling);
        }
        else if(m_sizeScaling < 1.0f)
        {
            ...
            if(!m_continuouslyTrigger)
                m_dSizeScaling = 0.0f;
            else m_dSizeScaling = fabsf(m_dSizeScaling);
        }
        
        int dw = m_width * m_sizeScaling, dh = m_height * m_sizeScaling;
        
        glViewport((m_width - dw) / 2, (m_height - dh) / 2, dw, dh);
        
        CGEMotionFlowFilter::pushFrame(texture);
    }
    
//    void CGEMotionFlow2Filter::render2Texture(CGEImageHandlerInterface *handler, GLuint srcTexture, GLuint vertexBufferID)
//    {
//        const CGESizei& sz = handler->getOutputFBOSize();
//        float alpha = 0.0f;
//        
