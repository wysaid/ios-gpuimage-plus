...
void main()
{
	float alphaGradient = texture2D(sAlphaTex, vTextureCoord).r;
	
	alphaGradient = smoothstep(alphaFactor.x, alphaFactor.y, alphaGradient);
//	clamp((alphaGradient - alphaFactor.x) / (alphaFactor.y - alphaFactor.x), 0.0, 1.0);
//	alphaGradient = alphaGradient * alphaGradient * (3.0 - 2.0 * alphaGradient);

	gl_FragColor = texture2D(sTexture, vTextureCoord);
)
#if CGE_TEXTURE_PREMULTIPLIED
	"gl_FragColor *= alphaGradient * alpha;"
#else
	"gl_FragColor.a *= alphaGradient * alpha;"
#endif
"}";
...
namespace CGE
{
	...
	{
		if(m_refCount == nullptr || m_pnt == nullptr)
		{
			CGE_LOG_ERROR("SharedPoint : Error occurred!\n");
			return;
		}
		...
		{
			//CGE_LOG_INFO("$$$ SharedPoint deRef count: %d\n", *m_refCount);
		}
	}

	void SharedPoint::clear()
	{
		assert(m_refCount != nullptr && *m_refCount == 0 && m_pnt != nullptr); //未知错误，请查证
...
// 	SpriteInterface2d::~SpriteInterface2d()
// 	{		
// 	}

	//////////////////////////////////////////////////////////////////////////

	Sprite2d* Sprite2d::sCreateWithBuffer(const void* bytes, int width, int height, CGEBufferFormat format)
	{
		int channel;
		...
		if(channel == 0)
			return nullptr;
		char* tmpBuffer = cgeGetScaledBufferInSize(bytes, width, height, channel, cgeGetMaxTextureSize(), cgeGetMaxTextureSize());
		const char* bufferData = (tmpBuffer == nullptr) ? (const char*)bytes : tmpBuffer;

		GLuint texID = cgeGenTextureWithBuffer(bufferData, width, height, channelFmt, dataFmt);
		delete[] tmpBuffer;
		return new Sprite2d(texID, width, height);
	}

	//////////////////////////////////////////////////////////////////////////

	ShaderObject *Sprite2d::spVertexShader = nullptr;
	ShaderObject *Sprite2d::spFragmentShader = nullptr;

	Sprite2d::Sprite2d(bool bInitProgram) : SpriteInterface2d() { assert(!bInitProgram); }
	Sprite2d::Sprite2d(GLuint textureID, int width, int height, bool bInitProgram) : SpriteInterface2d(), m_texture(textureID, width, height) { if(bInitProgram) {_initProgram();} }
	Sprite2d::Sprite2d(const SharedTexture& texture, bool bInitProgram) : SpriteInterface2d(), m_texture(texture) { if(bInitProgram) {_initProgram();} }

	Sprite2d::Sprite2d() { assert(0); } //兼容性接口

	Sprite2d::Sprite2d(GLuint textureID, int width, int height) : SpriteInterface2d(), m_texture(textureID, width, height) { _initProgram(); }
	Sprite2d::Sprite2d(const SharedTexture& texture) : SpriteInterface2d(), m_texture(texture) { _initProgram(); }
	Sprite2d::~Sprite2d() { _clearProgram(); }
	...
	void Sprite2d::_clearProgram()
	{
		//Seems like nothing to do...
	}

	bool Sprite2d::_tryLinkWithStaticShader()
	{
		return (Sprite2d::spVertexShader && Sprite2d::spFragmentShader) && m_program.linkWithShaderObject(*Sprite2d::spVertexShader, *Sprite2d::spFragmentShader, false);
	}
	...
	bool Sprite2d::_initProgram()
	{
		...
		if(!(Sprite2d::_tryLinkWithStaticShader() ||
			m_program.initWithShaderStrings(s_vshSprite2d, s_fshSprite2d)))
		{
			_clearProgram();
...
//		glUniform1i(m_textureLocation, 0);
	...
	ShaderObject *Sprite2dExt::spVertexShader = nullptr, *Sprite2dExt::spFragmentShader = nullptr;

	CGEConstString Sprite2dExt::getVertexString()
	{
		return s_vshSprite2dExt;
	}

	CGEConstString Sprite2dExt::getFragmentString()
	{
		return s_fshSprite2dExt;
	}

	Sprite2dExt::Sprite2dExt() { assert(0); }
	Sprite2dExt::Sprite2dExt(GLuint textureID, int width, int height) : Sprite2d(textureID, width, height, false), m_lineSprite(nullptr), m_blendMode(CGEGLOBAL_BLEND_NONE), m_lineWidth(4.0f) { _initProgram(); }
	Sprite2dExt::Sprite2dExt(const SharedTexture& texture) : Sprite2d(texture, false), m_lineSprite(nullptr), m_blendMode(CGEGLOBAL_BLEND_NONE), m_lineWidth(4.0f) { _initProgram(); }
	Sprite2dExt::~Sprite2dExt() { _clearProgram();}

	Vec2f Sprite2dExt::sZoneFlip = Vec2f(1.0f, -1.0f);
	ProgramObject* Sprite2dExt::sClipProgram = nullptr;

	bool Sprite2dExt::_tryLinkWithStaticShader()
	{
		return (Sprite2dExt::spVertexShader && Sprite2dExt::spFragmentShader) && m_program.linkWithShaderObject(*Sprite2dExt::spVertexShader, *Sprite2dExt::spFragmentShader, false);
	}

	bool Sprite2dExt::_initProgram()
	{
		m_posAttribLocation = 0;
		m_program.bindAttribLocation(paramAttribPositionName, m_posAttribLocation);
		...
		if(!(Sprite2dExt::_tryLinkWithStaticShader() ||
			m_program.initWithShaderStrings(s_vshSprite2dExt, s_fshSprite2dExt)))
		{
			_clearProgram();
			CGE_LOG_ERROR("Sprite2d - init program failed! ProgramID : %d\n", m_program.programID());
			return false;
		...
		m_scaleRatioLocation = m_program.uniformLocation("scaleRatio");
		glUniform2f(m_scaleRatioLocation, 1.0f, 1.0f);

		m_program.sendUniformf(paramBlendColorName, 1.0f, 1.0f, 1.0f);

		cgeCheckGLError("Sprite2d - initProgram");
		return true;
	}

	void Sprite2dExt::_clearProgram()
	{
		CGE_DELETE(m_lineSprite);
	}

	bool Sprite2dExt::clipZone()
	{
		if(m_zone.size() < 3)
		{
			CGE_LOG_CODE(
				if(!m_zone.empty())
					CGE_LOG_ERROR("Invalid Zone Number: %d\n!", (int)m_zone.size());
			);
			return false;
		}

		//Draw Wireframe
		std::vector<SharedPoint>::size_type sz = m_zone.size();

		if(m_dataCache.size() < sz * 2)
			m_dataCache.resize(sz * 2);

		m_lineSprite->clearPoints();
		for(std::vector<SharedPoint>::size_type i = 0, j = 0; i != sz; ++i, j += 2)
		{
			m_dataCache[j] = m_zone[i].pos().x();
			m_dataCache[j + 1] = m_zone[i].pos().y();

			m_lineSprite->pushPoint(m_zone[i].pos());
		}

		m_lineSprite->pushPoint(m_zone[0].pos());
		...
		glEnableVertexAttribArray(m_posAttribLocation);
		glVertexAttribPointer(m_posAttribLocation, 2, GL_FLOAT, false, 0, m_dataCache.data());
		...
		glDrawArrays(GL_TRIANGLE_FAN, 0, (GLsizei)m_zone.size());

		//设置clip状态
		glStencilFunc(GL_EQUAL, 1, 0xff);
		glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);

		cgeCheckGLError("Sprite2dExt::clipZone");
		return true;
	}

	void Sprite2dExt::restoreZone()
	{
		glDisable(GL_STENCIL_TEST);
	}

	bool Sprite2dExt::enableClip(bool flag)
	{		
		if(flag)
		{
			if(sClipProgram == nullptr)
			{
				sClipProgram = new ProgramObject;
				sClipProgram->bindAttribLocation(paramAttribPositionName, m_posAttribLocation);
				if(!sClipProgram->initWithShaderStrings(s_vshBlank, s_fshBlank))
				{
					_clearProgram();
					CGE_LOG_ERROR("Sprite2d - init program failed! ProgramID : %d\n", m_program.programID());
					return false;
			...
			m_lineSprite = new GeometryLineStrip2d();
			m_lineSprite->setLineWidth(m_lineWidth);
		}
		else
		{
			CGE_DELETE(m_lineSprite);
	...
	void Sprite2dExt::sReleaseClipProgram()
	{
		CGE_DELETE(sClipProgram);
	}

// 	void Sprite2dExt::render()
// 	{
// 		m_program.bind();
// 		glUniform2f(m_translationLocation, m_pos[0], m_pos[1]);
// 		glUniform2f(m_scalingLocation, m_scaling[0], m_scaling[1]);
...
// 	}
	...
	void Sprite2dExt::_drawFunc()
	{
		bool shouldClip = m_lineSprite && clipZone();
...
//		glUniform1i(m_textureLocation, 0);
	...
	void Sprite2dExt::setLineWidth(float lineWidth)
	{
		m_lineWidth = lineWidth;
		m_lineSprite->setLineWidth(lineWidth);
	}

	void Sprite2dExt::setLineColor(float r, float g, float b, float a)
	{
		m_lineSprite->setColor(r, g, b, a);
	}

	void Sprite2dExt::setGradient(float gradient)
	{
		m_lineSprite->setGradient(gradient);
	}

	//////////////////////////////////////////////////////////////////////////

	ShaderObject *Sprite2dWithAlphaGradient::spVertexShader = nullptr;
	ShaderObject *Sprite2dWithAlphaGradient::spFragmentShader = nullptr;
	...
	Sprite2dWithAlphaGradient::Sprite2dWithAlphaGradient() { assert(0); } //兼容性接口
	Sprite2dWithAlphaGradient::Sprite2dWithAlphaGradient(GLuint textureID, int width, int height) : Sprite2d(textureID, width, height, false) { _initProgram(); }
	Sprite2dWithAlphaGradient::Sprite2dWithAlphaGradient(const SharedTexture& texture) : Sprite2d(texture, false) { _initProgram(); }
	Sprite2dWithAlphaGradient::~Sprite2dWithAlphaGradient() { _clearProgram(); }
	...
	bool Sprite2dWithAlphaGradient::_initProgram()
	{
		...
		if(!(Sprite2dWithAlphaGradient::_tryLinkWithStaticShader() ||
			m_program.initWithShaderStrings(s_vshSprite2d, s_fshSprite2dWithSpecialAlpha)))
		{
			_clearProgram();
			CGE_LOG_ERROR("Sprite2d - init program failed! ProgramID : %d\n", m_program.programID());
			return false;
	...
	bool Sprite2dWithAlphaGradient::_tryLinkWithStaticShader()
	{
		return (Sprite2dWithAlphaGradient::spVertexShader && Sprite2dWithAlphaGradient::spFragmentShader) && m_program.linkWithShaderObject(*Sprite2dWithAlphaGradient::spVertexShader, *Sprite2dWithAlphaGradient::spFragmentShader, false);
	}

	void Sprite2dWithAlphaGradient::_clearProgram()
	{
...
//		glUniform1i(m_textureLocation, 0);
	...
	ShaderObject *Sprite2dInterChange::spVertexShader, *Sprite2dInterChange::spFragmentShader;
	...
	Sprite2dInterChange::Sprite2dInterChange() { assert(0); }
	Sprite2dInterChange::Sprite2dInterChange(GLuint textureID, int width, int height) : Sprite2d(textureID, width, height, false) { _initProgram(); }
	Sprite2dInterChange::Sprite2dInterChange(const SharedTexture& texture) : Sprite2d(texture, false) { _initProgram(); }
	Sprite2dInterChange::~Sprite2dInterChange() { _clearProgram(); }
	...
	bool Sprite2dInterChange::_initProgram()
	{
		...
		if(!(Sprite2dInterChange::_tryLinkWithStaticShader() ||
			m_program.initWithShaderStrings(s_vshSprite2dInterChange, s_fshSprite2d)))
		{
			_clearProgram();
	...
	bool Sprite2dInterChange::_tryLinkWithStaticShader()
	{
		return (Sprite2dInterChange::spVertexShader && Sprite2dInterChange::spFragmentShader) && m_program.linkWithShaderObject(*Sprite2dInterChange::spVertexShader, *Sprite2dInterChange::spFragmentShader, false);
	}

	void Sprite2dInterChange::_clearProgram()
	{
		//nothing to do...
	}
...
// 	}
    ...
    Sprite2dInterChangeMultiple::Sprite2dInterChangeMultiple(int width, int height) : Sprite2dInterChange(SharedTexture(width, height)) {}
...
//    void Sprite2dInterChangeMultiple::_drawFunc()
//    {
//        
//    }
...
//            frameIndex += tex.count;
	...
	ShaderObject *Sprite2dWith3dSpace::spVertexShader, *Sprite2dWith3dSpace::spFragmentShader;

	CGEConstString Sprite2dWith3dSpace::getVertexString()
	{
		return s_vshSprite2dWith3dSpace;
	}

	CGEConstString Sprite2dWith3dSpace::getFragmentString()
	{
		return s_fshSprite2d;
	}

	Sprite2dWith3dSpace::Sprite2dWith3dSpace() { assert(0); }

	Sprite2dWith3dSpace::Sprite2dWith3dSpace(GLuint textureID, int width, int height) : Sprite2d(textureID, width, height, false)
	{
		_initProgram();
	}

	Sprite2dWith3dSpace::Sprite2dWith3dSpace(const SharedTexture& texture) : Sprite2d(texture, false)
	{
		_initProgram();
	}

	Sprite2dWith3dSpace::~Sprite2dWith3dSpace()
	{
		_clearProgram();
	}

	bool Sprite2dWith3dSpace::_initProgram()
	{
		m_posAttribLocation = 0;
		m_program.bindAttribLocation(paramAttribPositionName, m_posAttribLocation);

		if(!(Sprite2dWith3dSpace::_tryLinkWithStaticShader() ||
			m_program.initWithShaderStrings(s_vshSprite2dWith3dSpace, s_fshSprite2d)))
		{
			_clearProgram();
			CGE_LOG_ERROR("Sprite2dWith3dSpace - init program failed! ProgramID : %d\n", m_program.programID());
			return false;
		}
	...
	bool Sprite2dWith3dSpace::_tryLinkWithStaticShader()
	{
		return (Sprite2dWith3dSpace::spVertexShader && Sprite2dWith3dSpace::spFragmentShader) && m_program.linkWithShaderObject(*Sprite2dWith3dSpace::spVertexShader, *Sprite2dWith3dSpace::spFragmentShader, false);
	}

	void Sprite2dWith3dSpace::_clearProgram()
	{
		
	}

	void Sprite2dWith3dSpace::render()
	{
		m_program.bind();
		glUniform2f(m_translationLocation, m_pos[0], m_pos[1]);
		glUniform2f(m_scalingLocation, m_scaling[0], m_scaling[1]);
		glUniformMatrix3fv(m_rotationLocation, 1, GL_FALSE, m_rotMatrix.data[0]);
		_drawFunc();
	}
